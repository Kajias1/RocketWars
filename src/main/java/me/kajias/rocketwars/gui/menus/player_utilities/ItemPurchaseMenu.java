...
public class ItemPurchaseMenu extends InventoryGUI
{
    ...
    private final ItemStack modifyingItem;
    private final List<Integer> pricesAnix;
    private final List<Integer> pricesBonus;
    private final List<String> benefit;
    private final String type;

    public ItemPurchaseMenu(ItemStack sellingItem, ItemStack modifyingItem, List<Integer> pricesAnix, List<Integer> pricesBonus, List<String> benefit, String type) {
        super(false);
                        ...
                        if (playerData != null) {
                            if (event.getClick().isRightClick() && event.getSlot() == sellingItemSlots.get(sellingItemSlots.size() - 1)) {
                                ...
                                if (!sufficientMoney) {
                                    ...
                                    switch (type) {
                                        case "bonus":
                                            playerData.getBoughtAbilities().put(Utils.stripColor(this.sellingItem.getItemMeta().getDisplayName()), period);
                                            Utils.sendMessage(player, config.getString("messages.item-purchase-successful"));
                                            break;
                                        case "armor-boots":
                                            playerData.getBoughtArmorItems().put(this.sellingItem, period);
                                            playerData.setBoots(this.sellingItem);
                                            Utils.sendMessage(player, config.getString("messages.item-purchase-successful"));
                                            break;
                                        case "armor-helmet":
                                            playerData.getBoughtArmorItems().put(this.sellingItem, period);
                                            playerData.setHelmet(this.sellingItem);
                                            Utils.sendMessage(player, config.getString("messages.item-purchase-successful"));
                                            break;
                                        case "color-boots":
                                        case "color-helmet":
                                            playerData.getBoughtColors().put(type + ":" + DyeColor.getByWoolData(this.sellingItem.getData().getData()).getColor().asRGB(), period);
                                            LeatherArmorMeta armorMeta = (LeatherArmorMeta) playerData.getBoughtArmorItem(modifyingItem).getItemMeta();
                                            armorMeta.setColor(Color.fromRGB(DyeColor.getByWoolData(sellingItem.getData().getData()).getColor().asRGB()));
                                            playerData.getBoughtArmorItem(modifyingItem).setItemMeta(armorMeta);
                                            playerData.setSelectedColorType(type, SelectedColorType.ONE);
                                            Utils.sendMessage(player, config.getString("messages.color-purchase-successful"));
                                            break;
                                        case "color-glass":
                                            playerData.getBoughtColors().put(type + ":" + DyeColor.getByWoolData(this.sellingItem.getData().getData()).getColor().asRGB(), period);
                                            playerData.setGlassColor(String.valueOf(DyeColor.getByWoolData(this.sellingItem.getData().getData())));
                                            playerData.setSelectedColorType(type, SelectedColorType.ONE);
                                            Utils.sendMessage(player, config.getString("messages.color-purchase-successful"));
                                            break;
                                    }
